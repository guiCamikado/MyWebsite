...
export default function Home() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      const getIp = async () => {
        try {
          const ipData = await ApiService.Get(
            "https://api.ipify.org?format=json",
          );
          await ApiService.Post("/api/ipManager/register", {
            ipAddress: ipData.ip,
          });
        } catch (error) {
          console.error(error);
      ...
      const differenceBetweenLastLogin =
        new Date().getTime() - new Date(data.recentConnection).getTime();
      const duasHoras = 1000 * 60 * 60 * 2;

      console.log(differenceBetweenLastLogin);
      if (
        !data.recentConnection ||
        differenceBetweenLastLogin > duasHoras ||
        isNaN(differenceBetweenLastLogin)
      ) {
        ...
        await FormDataManager.handleEventInput(
          { target: firstLoginToday },
          data,
          setData,
        ).then((result) => {
  ...
  const renderContent = () => {
    switch (renderedPage) {
      case "experience":
        return <ExperienceCard darkMode={data.darkMode} />;
      case "formation":
        return <EducationCard darkMode={data.darkMode} />;
      case "skills":
        return <SkillsSidebar darkMode={data.darkMode} />;
      case "projects":
        return <ProjectsSidebar darkMode={data.darkMode} />;
      case "login":
        return <LoginPage darkMode={data.darkMode} />;
      case "register":
        return <RegisterPage darkMode={data.darkMode} />;
      case "statistics":
        return <StatisticsPage darkMode={data.darkMode} />;
      default:
        return <SobreSidebar darkMode={data.darkMode} />;
    }
  };
